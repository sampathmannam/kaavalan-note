...
class Migration14To16Test {
    ...
    @Test fun `v16 reminders keep their exact time and do not become deadlines`() {
        ...
        helper.close()
        val db = Room.databaseBuilder(context, AppDatabase::class.java, dbPath).addMigrations(AppDatabase.MIGRATION_16_17, com.kaavalan.note.data.subdivision.SUBDIVISION_MIGRATION_17_18, AppDatabase.MIGRATION_18_19).build()
     ...
     */
    @Test
    fun `v15 database upgrades to v19 and passes Room schema validation`() {
        buildV15Fixture()

        val ctx = ApplicationProvider.getApplicationContext<android.content.Context>()
        val db = Room.databaseBuilder(ctx, AppDatabase::class.java, dbPath)
            .addMigrations(AppDatabase.MIGRATION_15_16, AppDatabase.MIGRATION_16_17, com.kaavalan.note.data.subdivision.SUBDIVISION_MIGRATION_17_18, AppDatabase.MIGRATION_18_19)
            .build()
        ...
        assertEquals("database should be at v19 after the migration chain", 19, version)
        db.close()
    }
     ...
     */
    @Test
    fun `v14 database upgrades through the full chain to v19`() {
        buildV14Fixture()

        val ctx = ApplicationProvider.getApplicationContext<android.content.Context>()
        val db = Room.databaseBuilder(ctx, AppDatabase::class.java, dbPath)
            .addMigrations(AppDatabase.MIGRATION_14_15, AppDatabase.MIGRATION_15_16, AppDatabase.MIGRATION_16_17, com.kaavalan.note.data.subdivision.SUBDIVISION_MIGRATION_17_18, AppDatabase.MIGRATION_18_19)
            .build()

        assertEquals(
            "database should be at v19 after the 14 -> 15 -> 16 -> 17 -> 18 -> 19 chain",
            19,
     ...
     *
     * The `v14 database upgrades through the full chain to v19` test
